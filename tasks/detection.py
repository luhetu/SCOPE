...
class DetectionTask:
    ...
    def _build_mmdet_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_default(getattr(args, "weight_decay", None), 0.05),
        ...
        cfg.log_config = dict(interval=int(_default(getattr(args, "log_interval", None), 50)), hooks=[dict(type="TextLoggerHook")])
        ...
        print(f"   lr: {args.lr}")
        print(f"   weight_decay: {_default(getattr(args, 'weight_decay', None), 0.05)}")
    ...
    def _get_backbone_config(self):
        ...
        common = dict(
            ...
            fpn_adapter_style=("simple_fpn" if str(_default(getattr(args, "det_neck_type", None), "fpn")).lower() == "fpn" else "identity"),
        )
        if args.model == "vit":
