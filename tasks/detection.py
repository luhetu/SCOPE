...
class DetectionTask:
    def __init__(self, args):
        ...
        if self.use_wandb:
            name = f"{self.run_name}_size{_get_arg(args, 'size', 'na')}_patch{args.patch}_dim{_get_arg(args, 'dim', 'na')}_bs{args.bs}_lr{args.lr}"
    ...
    def _build_mmdet_config(self):
        ...
        cfg.log_config = dict(interval=int(_get_arg(args, "log_interval", 50)), hooks=[dict(type="TextLoggerHook")])
    ...
    def _get_backbone_config(self):
        ...
        common = dict(
            image_size=_get_arg(args, "size", 224),
