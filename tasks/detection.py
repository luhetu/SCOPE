...
class DetectionTask:
    ...
    def _build_mmdet_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_get_or_default(args, "weight_decay", 0.05),
        ...
        cfg.log_config = dict(interval=int(_get_or_default(args, "log_interval", 50)), hooks=[dict(type="TextLoggerHook")])
        ...
        print(f"   lr: {args.lr}")
        print(f"   weight_decay: {_get_or_default(args, 'weight_decay', 0.05)}")
    ...
    def _get_mask_rcnn_config(self):
        neck_type = str(_get_or_default(self.args, "det_neck_type", "fpn")).lower()
        if neck_type == "fpn":
    ...
    def _get_backbone_config(self):
        ...
        common = dict(
            ...
            fpn_adapter_style=("simple_fpn" if str(_get_or_default(args, "det_neck_type", "fpn")).lower() == "fpn" else "identity"),
        )
        if args.model == "vit":
