...
class SegmentationTask:
    ...
    def _build_mmseg_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_default(getattr(args, "weight_decay", None), 0.01),
        ...
        if max_iters is None:
            max_iters = int(_default(getattr(args, "n_epochs", None), 32) * 1000)
        warmup_iters = getattr(args, "warmup_iters", None)
        if warmup_iters is None:
            warmup_iters = int(_default(getattr(args, "warmup_epochs", None), 0) * 1000)
        ...
        cfg.checkpoint_config = dict(
            by_epoch=False,
            interval=int(_default(getattr(args, "checkpoint_interval", None), 5000)),
            filename_tmpl=f"{self.run_name}_{args.task or 'seg'}_iter_{{}}.pth",
            max_keep_ckpts=3,
        )

        log_interval = int(_default(getattr(args, "log_interval", None), 100))
        eval_interval = int(_default(getattr(args, "eval_interval", None), 2001))
        ...
        cfg.custom_hooks = []
        cfg.final_eval = bool(_default(getattr(args, "final_eval", None), True))
        ...
        print(f"   lr: {args.lr}")
        print(f"   weight_decay: {_default(getattr(args, 'weight_decay', None), 0.01)}")
        print(f"   layer_decay_rate: {_default(getattr(args, 'layer_decay_rate', None), 1.0)}")
        print(f"   crop_size: {_default(getattr(args, 'crop_size', None), 512)}")
    ...
    def _get_paramwise_cfg(self):
        args = self.args
        layer_decay_rate = float(_default(getattr(args, "layer_decay_rate", None), 1.0))
        custom_keys = {
    ...
    def _get_upernet_config(self):
        ...
        head_dim = int(_default(getattr(self.args, "seg_head_dim", None), self._get_default_seg_head_dim()))
        aux_dim = int(_default(getattr(self.args, "seg_aux_dim", None), self._get_default_seg_head_dim()))
        aux_idx = int(_default(getattr(self.args, "seg_aux_in_index", None), 2))
        norm_cfg = self._get_seg_norm_cfg()

        neck_cfg = None
        seg_neck_style = str(_default(getattr(self.args, "seg_neck_style", None), "xcit_fpn")).lower()
        if self.args.model != "swin" and seg_neck_style in ("multilevel", "external"):
            neck_dim = int(_default(getattr(self.args, "seg_neck_dim", None), in_channels[0]))
    ...
    def _get_default_seg_head_dim(self):
        ...
        return int(_default(getattr(self.args, "dim", None), 512))

    def _get_seg_norm_cfg(self):
        norm_type = str(_default(getattr(self.args, "seg_norm_type", None), "SyncBN"))
    ...
    def _get_backbone_config(self):
        ...
        if args.model == "swin":
            return dict(
                ...
                drop_rate=0.0,
                attn_drop_rate=0.0,
                drop_path_rate=float(_default(getattr(args, "drop_path_rate", None), 0.0)),
                ape=False,
                patch_norm=True,
                out_indices=(0, 1, 2, 3),
        ...
        common = dict(
            ...
            dim_head=_default(getattr(args, "dim_head", None), 64),
            drop_path_rate=float(_default(getattr(args, "drop_path_rate", None), 0.0)),
            out_indices=tuple(_default(getattr(args, "out_indices", None), (3, 5, 7, 11))),
    ...
    def _get_vit_fpn_adapter_style(self):
        style = str(_default(getattr(self.args, "seg_neck_style", None), "xcit_fpn")).lower()
    ...
    def _get_data_config(self):
        ...
        return dict(
            ...
            workers_per_gpu=int(_default(getattr(args, "workers_per_gpu", None), 4)),
