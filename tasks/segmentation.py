...
def _as_int_arg(args, name, default):
    return int(_arg(args, name, default))


def _as_float_arg(args, name, default):
    return float(_arg(args, name, default))


def _as_bool_arg(args, name, default):
...
class SegmentationTask:
    ...
    def _build_mmseg_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_as_float_arg(args, "weight_decay", 0.01),
            paramwise_cfg=self._get_paramwise_cfg(),
        )
        cfg.optimizer_config = dict(grad_clip=None)

        if _as_bool_arg(args, "amp", False):
        ...
        if max_iters is None:
            max_iters = _as_int_arg(args, "n_epochs", 32) * 1000
        warmup_iters = getattr(args, "warmup_iters", None)
        if warmup_iters is None:
            warmup_iters = int(_as_float_arg(args, "warmup_epochs", 0) * 1000)
        ...
        cfg.lr_config = dict(
            ...
            warmup_ratio=1e-6,
            power=1.0,
            min_lr=_as_float_arg(args, "min_lr", 0.0),
            by_epoch=False,
        )
        cfg.checkpoint_config = dict(
            by_epoch=False,
            interval=_as_int_arg(args, "checkpoint_interval", 5000),
            filename_tmpl=f"{self.run_name}_{args.task or 'seg'}_iter_{{}}.pth",
            max_keep_ckpts=3,
        )

        log_interval = _as_int_arg(args, "log_interval", 100)
        eval_interval = _as_int_arg(args, "eval_interval", 2001)
        ...
        cfg.custom_hooks = []
        cfg.final_eval = _as_bool_arg(args, "final_eval", True)
        ...
        print(f"   lr: {args.lr}")
        print(f"   weight_decay: {_as_float_arg(args, 'weight_decay', 0.01)}")
        print(f"   layer_decay_rate: {_as_float_arg(args, 'layer_decay_rate', 1.0)}")
        print(f"   crop_size: {_arg(args, 'crop_size', 512)}")
        print(f"   backbone_image_size: {self._get_backbone_image_size()}")
        print(f"   seg_head_dim: {_as_int_arg(args, 'seg_head_dim', self._get_default_seg_head_dim())}")
        print(f"   seg_aux_dim: {_as_int_arg(args, 'seg_aux_dim', self._get_default_seg_head_dim())}")
    ...
    def _get_paramwise_cfg(self):
        args = self.args
        layer_decay_rate = _as_float_arg(args, "layer_decay_rate", 1.0)
        custom_keys = {
        ...
        if args.model != "swin" and layer_decay_rate < 1.0:
            depth = _as_int_arg(args, "depth", 12)
    ...
    def _get_upernet_config(self):
        ...
        head_dim = _as_int_arg(self.args, "seg_head_dim", self._get_default_seg_head_dim())
        aux_dim = _as_int_arg(self.args, "seg_aux_dim", self._get_default_seg_head_dim())
        aux_idx = _as_int_arg(self.args, "seg_aux_in_index", 2)
        ...
        if self.args.model != "swin" and seg_neck_style in ("multilevel", "external"):
            neck_dim = _as_int_arg(self.args, "seg_neck_dim", in_channels[0])
    ...
    def _get_default_seg_head_dim(self):
        ...
        return _as_int_arg(self.args, "dim", 512)
    ...
    def _get_backbone_image_size(self):
        value = _arg(self.args, "backbone_size", _arg(self.args, "crop_size", _arg(self.args, "size", 224)))
    ...
    def _get_backbone_config(self):
        ...
        if args.model == "swin":
            return dict(
                ...
                drop_rate=0.0,
                attn_drop_rate=0.0,
                drop_path_rate=_as_float_arg(args, "drop_path_rate", 0.0),
                ape=False,
                patch_norm=True,
                out_indices=(0, 1, 2, 3),
        ...
        common = dict(
            ...
            dim_head=_as_int_arg(args, "dim_head", 64),
            drop_path_rate=_as_float_arg(args, "drop_path_rate", 0.0),
        ...
        if args.model == "vitcope":
            return dict(type="ViTCoPEBackbone", use_cls_token=_as_bool_arg(args, "use_cls_token", False), **common)
        if args.model == "vitscope":
            return dict(type="ViTSCoPEBackbone", use_cls_token=_as_bool_arg(args, "use_cls_token", True), **common)
    ...
    def _get_data_config(self):
        ...
        return dict(
            ...
            workers_per_gpu=_as_int_arg(args, "workers_per_gpu", 4),
    ...
    def _load_pretrained_backbone(self, pretrained_path):
        ...
        if rate < _as_float_arg(self.args, "min_pretrained_match_rate", 80.0):
