...
def _float_arg(args, name, default):
    return float(_arg_or_default(args, name, default))


def _bool_arg(args, name, default):
    return bool(_arg_or_default(args, name, default))


def _str_arg(args, name, default):
    return str(_arg_or_default(args, name, default))
...
class SegmentationTask:
    ...
    def _build_mmseg_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_float_arg(args, "weight_decay", 0.01),
        ...
        cfg.lr_config = dict(
            ...
            warmup_ratio=1e-6,
            power=1.0,
            min_lr=_float_arg(args, "min_lr", 0.0),
            by_epoch=False,
        )
        ...
        cfg.custom_hooks = []
        cfg.final_eval = _bool_arg(args, "final_eval", True)
        ...
        print(f"   lr: {args.lr}")
        print(f"   weight_decay: {_float_arg(args, 'weight_decay', 0.01)}")
        print(f"   layer_decay_rate: {_float_arg(args, 'layer_decay_rate', 1.0)}")
        ...
        print(f"   seg_norm_type: {_str_arg(args, 'seg_norm_type', 'SyncBN')}")
        print(f"   work_dir: {cfg.work_dir}")
        return cfg

    def _get_paramwise_cfg(self):
        args = self.args
        layer_decay_rate = _float_arg(args, "layer_decay_rate", 1.0)
        custom_keys = {
    ...
    def _get_upernet_config(self):
        ...
        neck_cfg = None
        seg_neck_style = _str_arg(self.args, "seg_neck_style", "xcit_fpn").lower()
    ...
    def _get_default_seg_head_dim(self):
        ...
        return _int_arg(self.args, "dim", 512)

    def _get_seg_norm_cfg(self):
        norm_type = _str_arg(self.args, "seg_norm_type", "SyncBN")
    ...
    def _get_backbone_config(self):
        ...
        if args.model == "swin":
            return dict(
                ...
                drop_rate=0.0,
                attn_drop_rate=0.0,
                drop_path_rate=_float_arg(args, "drop_path_rate", 0.0),
                ape=False,
                patch_norm=True,
                out_indices=(0, 1, 2, 3),
        ...
        if args.model == "vitcope":
            return dict(type="ViTCoPEBackbone", use_cls_token=_bool_arg(args, "use_cls_token", False), **common)
        if args.model == "vitscope":
            return dict(type="ViTSCoPEBackbone", use_cls_token=_bool_arg(args, "use_cls_token", True), **common)
        raise ValueError(f"Unknown segmentation backbone model: {args.model}")

    def _get_vit_fpn_adapter_style(self):
        style = _str_arg(self.args, "seg_neck_style", "xcit_fpn").lower()
    ...
    def _load_pretrained_backbone(self, pretrained_path):
        ...
        if rate < _float_arg(self.args, "min_pretrained_match_rate", 80.0):
