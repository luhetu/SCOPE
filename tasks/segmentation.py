...
class SegmentationTask:
    ...
    def _build_mmseg_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_get_arg(args, "weight_decay", 0.01),
        ...
        cfg.lr_config = dict(
            ...
            warmup_ratio=1e-6,
            power=1.0,
            min_lr=_get_arg(args, "min_lr", 0.0),
            by_epoch=False,
        )
        cfg.checkpoint_config = dict(
            by_epoch=False,
            interval=int(_get_arg(args, "checkpoint_interval", 5000)),
            filename_tmpl=f"{self.run_name}_{args.task or 'seg'}_iter_{{}}.pth",
            max_keep_ckpts=3,
        )

        log_interval = int(_get_arg(args, "log_interval", 100))
        eval_interval = int(_get_arg(args, "eval_interval", 2001))
        ...
        cfg.custom_hooks = []
        cfg.final_eval = bool(_get_arg(args, "final_eval", True))
        ...
        cfg.seed = _get_arg(args, "seed", None)
    ...
    def _get_paramwise_cfg(self):
        args = self.args
        layer_decay_rate = float(_get_arg(args, "layer_decay_rate", 1.0))
        custom_keys = {
        ...
        if args.model != "swin" and layer_decay_rate < 1.0:
            depth = int(_get_arg(args, "depth", 12))
    ...
    def _get_upernet_config(self):
        ...
        head_dim = int(_get_arg(self.args, "seg_head_dim", self._get_default_seg_head_dim()))
        aux_dim = int(_get_arg(self.args, "seg_aux_dim", self._get_default_seg_head_dim()))
        aux_idx = int(_get_arg(self.args, "seg_aux_in_index", 2))
    ...
    def _get_default_seg_head_dim(self):
        ...
        return int(_get_arg(self.args, "dim", 512))

    def _get_seg_norm_cfg(self):
        norm_type = str(_get_arg(self.args, "seg_norm_type", "SyncBN"))
    ...
    def _get_backbone_image_size(self):
        value = _get_arg(self.args, "backbone_size", _get_arg(self.args, "crop_size", self.args.size))
    ...
    def _get_backbone_config(self):
        ...
        if args.model == "vitcope":
            return dict(type="ViTCoPEBackbone", use_cls_token=bool(_get_arg(args, "use_cls_token", False)), **common)
        if args.model == "vitscope":
            return dict(type="ViTSCoPEBackbone", use_cls_token=bool(_get_arg(args, "use_cls_token", True)), **common)
    ...
    def _get_data_config(self):
        ...
        return dict(
            ...
            workers_per_gpu=int(_get_arg(args, "workers_per_gpu", 4)),
    ...
    def _map_pretrained_key(self, key):
        ...
        last_norm_idx = len(_get_arg(self.args, "out_indices", (3, 5, 7, 11))) - 1
    ...
    def _load_pretrained_backbone(self, pretrained_path):
        ...
        if rate < float(_get_arg(self.args, "min_pretrained_match_rate", 80.0)):
