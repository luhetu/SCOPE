...
def _arg_or_default(args, name, default):
    return _value_or_default(getattr(args, name, None), default)


def _as_bool(value, default=False):
    return bool(_value_or_default(value, default))
...
class SegmentationTask:
    def __init__(self, args):
        ...
        self.use_wandb = WANDB_AVAILABLE and not _as_bool(getattr(args, "nowandb", None), False)
        ...
        elif not WANDB_AVAILABLE and not _as_bool(getattr(args, "nowandb", None), False):
    ...
    def _build_mmseg_config(self):
        ...
        if _as_bool(getattr(args, "amp", None), False):
        ...
        if max_iters is None:
            max_iters = int(_arg_or_default(args, "n_epochs", 32) * 1000)
        ...
        cfg.lr_config = dict(
            ...
            warmup_ratio=1e-6,
            power=1.0,
            min_lr=_arg_or_default(args, "min_lr", 0.0),
            by_epoch=False,
        )
        ...
        cfg.custom_hooks = []
        cfg.final_eval = _as_bool(getattr(args, "final_eval", None), True)
    ...
    def _get_paramwise_cfg(self):
        ...
        if args.model != "swin" and layer_decay_rate < 1.0:
            depth = int(_arg_or_default(args, "depth", 12))
    ...
    def _get_backbone_image_size(self):
        value = _arg_or_default(self.args, "backbone_size", None)
        if value is None:
            value = _arg_or_default(self.args, "crop_size", self.args.size)
    ...
    def _get_backbone_config(self):
        ...
        if args.model == "vitcope":
            return dict(type="ViTCoPEBackbone", use_cls_token=_as_bool(getattr(args, "use_cls_token", None), False), **common)
        if args.model == "vitscope":
            return dict(type="ViTSCoPEBackbone", use_cls_token=_as_bool(getattr(args, "use_cls_token", None), True), **common)
