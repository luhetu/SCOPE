...
class SegmentationTask:
    ...
    def _build_mmseg_config(self):
        ...
        cfg.optimizer = dict(
            ...
            weight_decay=_arg_value(args, "weight_decay", 0.01),
        ...
        if max_iters is None:
            max_iters = int(_arg_value(args, "n_epochs", 32) * 1000)
        warmup_iters = getattr(args, "warmup_iters", None)
        if warmup_iters is None:
            warmup_iters = int(_arg_value(args, "warmup_epochs", 0) * 1000)
        ...
        cfg.checkpoint_config = dict(
            by_epoch=False,
            interval=int(_arg_value(args, "checkpoint_interval", 5000)),
            filename_tmpl=f"{self.run_name}_{args.task or 'seg'}_iter_{{}}.pth",
            max_keep_ckpts=3,
        )

        log_interval = int(_arg_value(args, "log_interval", 100))
        eval_interval = int(_arg_value(args, "eval_interval", 2001))
        ...
        print(f"   lr: {args.lr}")
        print(f"   weight_decay: {_arg_value(args, 'weight_decay', 0.01)}")
        print(f"   layer_decay_rate: {_arg_value(args, 'layer_decay_rate', 1.0)}")
        print(f"   crop_size: {_arg_value(args, 'crop_size', 512)}")
        print(f"   backbone_image_size: {self._get_backbone_image_size()}")
        print(f"   seg_head_dim: {_arg_value(args, 'seg_head_dim', self._get_default_seg_head_dim())}")
        print(f"   seg_aux_dim: {_arg_value(args, 'seg_aux_dim', self._get_default_seg_head_dim())}")
        print(f"   seg_norm_type: {_arg_value(args, 'seg_norm_type', 'SyncBN')}")
        print(f"   work_dir: {cfg.work_dir}")
        return cfg

    def _get_paramwise_cfg(self):
        args = self.args
        layer_decay_rate = float(_arg_value(args, "layer_decay_rate", 1.0))
        custom_keys = {
        ...
        if args.model != "swin" and layer_decay_rate < 1.0:
            depth = int(_arg_value(args, "depth", 12))
    ...
    def _get_upernet_config(self):
        ...
        head_dim = int(_arg_value(self.args, "seg_head_dim", self._get_default_seg_head_dim()))
        aux_dim = int(_arg_value(self.args, "seg_aux_dim", self._get_default_seg_head_dim()))
        aux_idx = int(_arg_value(self.args, "seg_aux_in_index", 2))
        norm_cfg = self._get_seg_norm_cfg()

        neck_cfg = None
        seg_neck_style = str(_arg_value(self.args, "seg_neck_style", "xcit_fpn")).lower()
        if self.args.model != "swin" and seg_neck_style in ("multilevel", "external"):
            neck_dim = int(_arg_value(self.args, "seg_neck_dim", in_channels[0]))
    ...
    def _get_default_seg_head_dim(self):
        ...
        return int(_arg_value(self.args, "dim", 512))

    def _get_seg_norm_cfg(self):
        norm_type = str(_arg_value(self.args, "seg_norm_type", "SyncBN"))
    ...
    def _get_backbone_image_size(self):
        value = _arg_value(self.args, "backbone_size", _arg_value(self.args, "crop_size", self.args.size))
    ...
    def _get_backbone_config(self):
        ...
        if args.model == "swin":
            return dict(
                ...
                drop_rate=0.0,
                attn_drop_rate=0.0,
                drop_path_rate=float(_arg_value(args, "drop_path_rate", 0.0)),
                ape=False,
                patch_norm=True,
                out_indices=(0, 1, 2, 3),
        ...
        common = dict(
            ...
            dim_head=_arg_value(args, "dim_head", 64),
            drop_path_rate=float(_arg_value(args, "drop_path_rate", 0.0)),
            out_indices=tuple(_arg_value(args, "out_indices", (3, 5, 7, 11))),
            fpn_adapter_style=self._get_vit_fpn_adapter_style(),
        )
        if args.model == "vit":
            return dict(type="ViTBackbone", **common)
        if args.model == "vitcope":
            return dict(type="ViTCoPEBackbone", use_cls_token=bool(_arg_value(args, "use_cls_token", False)), **common)
        if args.model == "vitscope":
            return dict(type="ViTSCoPEBackbone", use_cls_token=bool(_arg_value(args, "use_cls_token", True)), **common)
        raise ValueError(f"Unknown segmentation backbone model: {args.model}")

    def _get_vit_fpn_adapter_style(self):
        style = str(_arg_value(self.args, "seg_neck_style", "xcit_fpn")).lower()
    ...
    def _get_data_config(self):
        ...
        return dict(
            ...
            workers_per_gpu=int(_arg_value(args, "workers_per_gpu", 4)),
