...
class EvalHook(Hook):
    ...
    def save_best_checkpoint(self, runner, key_score):
        ...
        best_score = hook_msgs.get('best_score', self.init_value_map[self.rule])
        if self.compare_func(key_score, best_score):
            best_score = key_score
            hook_msgs['best_score'] = best_score
            last_ckpt = hook_msgs.get('last_ckpt')
            if last_ckpt is None:
                self.logger.warning('last_ckpt not found, skip best checkpoint symlink.')
                return
            hook_msgs['best_ckpt'] = last_ckpt
            mmcv.symlink(
                last_ckpt,
